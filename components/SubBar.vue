...
function saveModal() {
  ...
  if (editingIndex.value >= 0) links.value[editingIndex.value] = { name, url };
  else links.value.push({ name, url });
  storeSet(K_LINKS, links.value);
  closeModal();
}
    ...
    <Transition name="subswap" mode="out-in">
      <div v-if="subbarView === 'links'" key="links" class="sub-swap">
        <div class="links-pills">
          <span v-for="(link, idx) in links" :key="link.url + idx" class="pill">
            <button class="pill-inner" :title="link.url" @click="openLink(link)">
